...
class Pengumuman {
  ...
  factory Pengumuman.fromJson(String id, Map<String, dynamic> json) {
    return Pengumuman(
      id: id,
      judul: json['judul'] ?? '',
      konten: json['konten'] ?? '',
      ...
      targetAudience: List<String>.from(json['targetAudience'] ?? ['semua']),
      tanggalPost:
          (json['tanggalPost'] as Timestamp?)?.toDate() ?? DateTime.now(),
      tanggalExpired: (json['tanggalExpired'] as Timestamp?)?.toDate(),
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'tanggalPost': Timestamp.fromDate(tanggalPost),
      'tanggalExpired': tanggalExpired != null
          ? Timestamp.fromDate(tanggalExpired!)
          : null,
      'isActive': isActive,
      'authorId': authorId,
...
/// Provider untuk daftar pengumuman
final pengumumanProvider = StreamProvider<List<Pengumuman>>((ref) {
...
class _AnnouncementManagementPageState
  ...
  Widget _buildPengumumanList(List<Pengumuman> pengumumanList) {
    // Apply filters
  ...
  Future<void> _addPengumuman(WidgetRef ref, Pengumuman pengumuman) async {
    try {
  ...
  Future<void> _toggleActiveStatus(WidgetRef ref, Pengumuman pengumuman) async {
    try {
  ...
  Future<void> _resendNotification(Pengumuman pengumuman) async {
    try {
      await _sendPengumumanNotification(pengumuman);
    } catch (e) {
  ...
  Future<void> _sendPengumumanNotification(Pengumuman pengumuman) async {
    try {
